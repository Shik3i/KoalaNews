export const DESIGN_OPTIONS = ['clean', 'newspaper', 'terminal', 'soft', 'high-contrast'] as const;
...
export const ACCENT_OPTIONS = ['blue', 'green', 'red', 'neutral'] as const;
...
export function normalizeAppearance(input: Record<string, unknown>): AppearanceSettings {
  return {
    ...
    showImages: typeof input.showImages === 'boolean' ? input.showImages : DEFAULT_APPEARANCE.showImages,
    showSource: typeof input.showSource === 'boolean' ? input.showSource : DEFAULT_APPEARANCE.showSource,
    ...
    showReadMore:
      typeof input.showReadMore === 'boolean' ? input.showReadMore : DEFAULT_APPEARANCE.showReadMore,
    descriptionLines:
