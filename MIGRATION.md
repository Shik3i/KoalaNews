...
- [x] **Post-Parity UX-Polish (2026-06-27)** — Appearance-System wieder auf Legacy-Tiefe und darüber hinaus gebracht (Design-Skins, dedizierte `/appearance`-Seite, große Live-Vorschau, Server-Persistenz der neuen Achsen). Feed-UX skaliert jetzt für große Abo-Listen: kurze/umbenennbare Feed-Namen (`custom_title` + Auto-Shortening), wiederverwendbarer Such-/Checkbox-FeedPicker, ad-hoc Multi-Feed-Filter auf der Home-Page, Speichern als Custom Feed, Dashboard-Form für Custom Feeds aus Feed-Auswahl + optionalem Keyword. **Verifiziert:** `npm run check`, `npm run build`, `go vet ./...`, `go test ./...`, Binary-Smoke-Test mit Registrierung, 2 Feeds, `?feeds=`, `?smartFeed=`, Feed-Rename und Browser-Check Home/Dashboard.
